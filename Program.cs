...
async Task MainAsync(string[] args)
{
    ...
    bool verbose = pargs.Has("--verbose");
    bool skipTitle = pargs.Has("--no-title");
    ...
    if (pargs.IsCommand("version"))
    {
        Console.WriteLine($"Version: {Version}");
        return;
    }

    if (pargs.IsCommand("update"))
    {
        await RunUpdate();
        return;
    }

    if (pargs.IsCommand("help") || pargs.IsCommand(""))
    {
        ShowHelp();
        return;
    }

    logger.Error($"Unknown command: {pargs.GetCommand()}. Use 'ranger help' for usage information.");
    return;
    ...
    void ShowHelp()
    {
        Console.WriteLine("Usage: ranger [command] [options]");
        Console.WriteLine("Commands:");
        Console.WriteLine("  update    - Update repositories to the versions specified in the ranger.json file.");
        Console.WriteLine("  version   - Display the version of the application.");
        Console.WriteLine("  help      - Display this help message.");
        Console.WriteLine("");
        Console.WriteLine("Common Options:");
        Console.WriteLine("  --base-dir <path>  - The base directory to use. Default is the current directory.");
        Console.WriteLine("  --json <path>      - The path to the ranger.json file. Default is <base-dir>/ranger.json.");
        Console.WriteLine("  --auth <path>      - The path to the ranger-auth.json file. Default is <base-dir>/ranger-auth.json.");
        Console.WriteLine("  --lock <path>      - The path to the ranger.lock file. Default is <base-dir>/ranger.lock.");
        Console.WriteLine("  --verbose          - Enable verbose logging.");
        Console.WriteLine("  --no-title         - Skip the application splash screen.");
        Console.WriteLine("");
    }
}